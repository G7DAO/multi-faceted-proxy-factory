...
const log: debug.Debugger = debug("G7DAOUpgrade:log");
...
async function attachG7DAODiamond (networkDeployInfo: INetworkDeployInfo) {
  ...
  dc._G7DAODiamond = (await ethers.getContractFactory("contracts/G7DAODiamond.sol:G7DAODiamond"))
    .attach(diamondAddress);
  dc.G7DAODiamond = (await ethers.getContractFactory("hardhat-diamond-abi/G7DAODiamond.sol:G7DAODiamond"))
    .attach(diamondAddress);
...
async function main () {
  ...
  if (require.main === module) {
    ...
    if (networkName in deployments) {
      ...
      await attachG7DAODiamond(deployInfo);
